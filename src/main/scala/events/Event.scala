...
object OrderEvent {
  ...
  implicit final class OrderEventOps[+O <: Output](val event: OrderEvent[O]) extends AnyVal {
    def projection(implicit eventLog: EventStore[String]): Order = {
      eventLog.get(event.id).filter(evt => evt.at <= this.event.at).foldRight(Order("", List.empty, None))((evt, order) => evt.asInstanceOf[OrderEvent[_]].entity match {
        case None => order
